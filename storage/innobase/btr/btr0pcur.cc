...
@param mtr mtr
@return  btr_pcur_t::NOT_SAME  cursor position is not on user record or
                               points on the record with not the same
                               uniq field values as in the stored
                               record,
         btr_pcur_t::SAME_ALL  cursor position on user record and points on
                               the record with the same field values as
                               in the stored record,
         btr_pcur_t::SAME_UNIQ cursor position is on user record and
                             ...
                             unsigned line, mtr_t *mtr)
{
	...
	index = btr_cur_get_index(btr_pcur_get_btr_cur(this));
	...
	if (UNIV_UNLIKELY
		...
		err = btr_cur_open_at_index_side(
			...
			btr_pcur_get_btr_cur(this), 0, mtr);
	...
	rec_offs_init(offsets);
        ...
        if (rel_pos == BTR_PCUR_ON && btr_pcur_is_on_user_rec(this))
        {
                  ...
                  &n_matched_fields))
          {
            ...
            modify_clock=
                buf_block_get_modify_clock(block_when_stored.block());
            old_stored= true;
