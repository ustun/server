...
struct btr_pcur_t{
        ...
        enum restore_status
        {
          /** cursor position is not on user rec or points on the record with
          not the same uniq field values as in the stored record */
          NOT_SAME,
          /** cursor position on user rec and points on the record with the
          same field values as in the stored record */
          SAME_ALL,
	...
	dict_index_t*	index() const { return(btr_cur.index); }
        ...
        @param mtr mtr
        @return  btr_pcur_t::NOT_SAME  cursor position is not on user rec or
                                       points on the record with not the same
                                       uniq field values as in the stored
                                       record,
                 btr_pcur_t::SAME_ALL  cursor position on user rec and points on
                                       the record with the same field values as
                                       in the stored record,
                 btr_pcur_t::SAME_UNIQ cursor position is on user rec and
                                       points on the record with the same unique
                                       field values as in the stored record. */
